			...
			<div className="flex items-center gap-3 flex-1 min-w-0">
				...
				<div className="flex-1 min-w-0">
					<div className="flex items-center gap-2">
						<span className={cn("font-medium text-sm", mode.disabled && "line-through")}>{mode.name}</span>
						<Badge
							variant="outline"
							className={cn("text-xs px-1.5 py-0.5", SOURCE_INFO[mode.source].color)}>
							{SOURCE_INFO[mode.source].icon} {mode.source}
						</Badge>
					</div>
					{mode.description && (
						<p className={cn("text-xs text-gray-600 mt-1 truncate", mode.disabled && "opacity-50")}>
							{mode.description}
						</p>
					)}
					<div className="text-xs text-gray-500 mt-0.5">slug: {mode.slug}</div>
				</div>
			</div>
